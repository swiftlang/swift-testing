...
extension Array where Element == PackageDescription.SwiftSetting {
  ...
  static func packageSettings(isTestTarget: Bool = false) -> Self {
    ...
    result += [
      ...
      .define("SWT_NO_EXIT_TESTS", .whenEmbedded(or: .when(platforms: [.iOS, .watchOS, .tvOS, .visionOS, .wasi, .custom("emscripten"), .android]))),
      .define("SWT_NO_PROCESS_SPAWNING", .whenEmbedded(or: .when(platforms: [.iOS, .watchOS, .tvOS, .visionOS, .wasi, .custom("emscripten"), .android]))),
      .define("SWT_NO_SNAPSHOT_TYPES", .whenEmbedded(or: .whenApple(false))),
      .define("SWT_NO_DYNAMIC_LINKING", .whenEmbedded(or: .when(platforms: [.wasi, .custom("emscripten")]))),
      .define("SWT_NO_PIPES", .whenEmbedded(or: .when(platforms: [.wasi, .custom("emscripten")]))),
      .define("SWT_NO_FOUNDATION_FILE_COORDINATION", .whenEmbedded(or: .whenApple(false))),
      .define("SWT_NO_IMAGE_ATTACHMENTS", .whenEmbedded(or: .when(platforms: [.linux, .custom("freebsd"), .openbsd, .wasi, .custom("emscripten"), .android]))),
      .define("SWT_NO_FILE_CLONING", .whenEmbedded(or: .when(platforms: [.openbsd, .wasi, .custom("emscripten"), .android]))),
