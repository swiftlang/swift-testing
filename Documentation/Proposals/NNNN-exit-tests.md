...
### Support for signals on Windows

Windows emulates UNIX-like signal handling in order to meet the requirements of
the C standard, but it doesn't truly support them. When a process on Windows
terminates due to an unhandled signal, it simply exits with exit code `3`. It
may be possible to install signal handlers on Windows that force exiting in a
way that the parent process can detect and interpret as a specific unhandled
signal.
