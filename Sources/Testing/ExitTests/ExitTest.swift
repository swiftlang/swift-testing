...
#warning("Platform-specific implementation missing: additional file descriptors unavailable")
  ...
  private static func _processRecords(fromBackChannel backChannel: borrowing FileHandle) {
    do {
      while !backChannel.isAtEnd {
        let recordJSON = try backChannel.read(until: \.isASCIINewline)
  ...
  private mutating func _decodeCapturedValuesForEntryPoint() throws {
    ...
    while !fileHandle.isAtEnd {
      let capturedValueJSON = try fileHandle.read(until: \.isASCIINewline)
      if !capturedValueJSON.isEmpty {
        capturedValuesJSONLines.append(capturedValueJSON)
      }
    }
