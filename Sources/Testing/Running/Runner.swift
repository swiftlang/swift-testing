...
extension Runner {
  ...
  private static func _applyScopingTraits(
    ...
    if test.isSerialized {
      // One of the traits applied above will be responsible for serializing.
      try await executeAllTraits()
    } else if !test.isSuite {
      // Run this test function as if it's in the globally-serialized work group
      // but concurrently with other tests.
      try await ParallelizationTrait.workGroup(for: .globally).run {
        try await executeAllTraits()
      }
    }
  }
  ...
  private static func _runChildren(of stepGraph: Graph<String, Plan.Step?>) async throws {
    ...
    let childGraphs = stepGraph.children.sorted { lhs, rhs in
