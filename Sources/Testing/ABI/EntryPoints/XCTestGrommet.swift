...
package struct XCTestGrommet: Grommet {
  ...
  package func run(_ eventHandler: @escaping @Sendable (borrowing Event, borrowing Event.Context) -> Void) async throws {
    try await withThrowingTaskGroup(of: Void.self) { taskGroup in
      ...
      eventHandler(Event(.runStarted, testID: nil, testCaseID: nil), context)
      defer {
        eventHandler(Event(.runEnded, testID: nil, testCaseID: nil), context)
      }

      // Read events back out from the back channel.
      taskGroup.addTask(name: decorateTaskName("harness", withAction: "reading output from XCTest")) {
        ...
        repeat {
          ...
          try _handleLine(line, eventHandler: eventHandler)
  ...
  private func _handleLine(_ line: [UInt8], eventHandler: @Sendable (borrowing Event, borrowing Event.Context) -> Void) throws {
