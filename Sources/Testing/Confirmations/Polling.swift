...
public struct PollingStopCondition: Sendable, Equatable, Codable {
  ...
  /// If the expression reports any issues, then a failure will be reported.
...
private struct Poller {
  ...
  /// A ``SourceLocation`` indicating where this confirmation was called.
  ...
  private func poll<R>(
    ...
    for iteration in 0..<iterations {
      let (value, issues): (R?, [Issue]) = await captureIssues(
        silently: true,
        sourceLocation: sourceLocation
      ) {
        do {
