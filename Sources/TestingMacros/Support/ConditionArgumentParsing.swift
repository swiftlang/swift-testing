...
/// The set of syntax node kinds that might be rewritten by an instance of the
/// `_ContextInserter` class.
///
/// The members of this set determine which nodes, when visited, contribute to
/// the IDs generated by `expressionIDBuilder`. By limiting the set of
/// contributing nodes to those that we might rewrite, we avoid wasting bits on
/// nodes that won't be rewritten.
///
/// - Important: If you add a new overload of `visit(_:)`, you _must_ update
///   this set to include the visited node kind.
private let _rewrittenNodeKinds: Set<SyntaxKind> = [
  .declReferenceExpr, .tupleExpr, .memberAccessExpr, .functionCallExpr,
  .subscriptCallExpr, .closureExpr, .functionDecl, .prefixOperatorExpr,
  .infixOperatorExpr, .inOutExpr, .asExpr, .isExpr, .booleanLiteralExpr,
  .integerLiteralExpr, .floatLiteralExpr, .stringLiteralExpr, .arrayExpr,
  .dictionaryExpr,
]
...
private final class _ContextInserter<C, M>: SyntaxRewriter where C: MacroExpansionContext, M: FreestandingMacroExpansionSyntax {
  ...
  /// A structure responsible for constructing unique IDs for any rewritten
  /// expressions.
  var expressionIDBuilder: ExpressionIDBuilder<C>

  init(in context: C, for macro: M, expressionIDBuilder: ExpressionIDBuilder<C>, expressionContextName: TokenSyntax) {
    self.context = context
    self.macro = macro
    self.expressionIDBuilder = expressionIDBuilder
  ...
  private func _rewrite<E>(_ node: E, originalWas originalNode: some SyntaxProtocol) -> ExprSyntax where E: ExprSyntaxProtocol {
    ...
    var result = FunctionCallExprSyntax(calledExpression: expressionContextNameExpr) {
      ...
      LabeledExprSyntax(expression: expressionIDBuilder.idExpression(for: originalNode))
    }
  ...
  override func visitPre(_ node: Syntax) {
    if _rewrittenNodeKinds.contains(node.kind) {
      expressionIDBuilder.memoize(node)
    }
  }
  ...
  // MARK: - Calls (and call-like things)
  ...
  override func visit(_ node: InfixOperatorExprSyntax) -> ExprSyntax {
    if let op = node.operator.as(BinaryOperatorExprSyntax.self)?.operator.textWithoutBackticks,
      ...
      var result = FunctionCallExprSyntax(
        ...
        LabeledExprSyntax(expression: expressionIDBuilder.idExpression(for: node.leftOperand))
        LabeledExprSyntax(expression: visit(node.rightOperand))
        LabeledExprSyntax(expression: expressionIDBuilder.idExpression(for: node.rightOperand))
        LabeledExprSyntax(
        ...
        LabeledExprSyntax(expression: expressionIDBuilder.idExpression(for: node))
      }
      result.leftParen = .leftParenToken()
      result.rightParen = .rightParenToken()
  ...
  private func _makeCastCall(_ valueExpr: ExprSyntax, _ isAsKeyword: Keyword, _ type: TypeSyntax) -> FunctionCallExprSyntax {
    var result = FunctionCallExprSyntax(
      ...
      LabeledExprSyntax(expression: expressionIDBuilder.idExpression(for: type))
    }
    result.leftParen = .leftParenToken()
    result.rightParen = .rightParenToken()
...
///   - expressionIDBuilder: A structure responsible for constructing unique IDs
///     for any rewritten expressions.
///   - context: The macro context in which the expression is being parsed.
///
...
func insertCalls<C>(
  ...
  buildingExpressionIDsWith expressionIDBuilder: inout ExpressionIDBuilder<C>,
  in context: C
  ...
  let contextInserter = _ContextInserter(in: context, for: macro, expressionIDBuilder: expressionIDBuilder, expressionContextName: expressionContextName)
  let result = contextInserter.rewrite(node)
  expressionIDBuilder = contextInserter.expressionIDBuilder
