      ...
      if let trailingClosureIndex {
        ...
        if !useEscapeHatch {
          ...
          if let (closureExpr, rewrittenNodes) = resultOfRewrite {
            ...
            let expressionCapacity = rewrittenNodes.count
            if expressionCapacity > defaultExpectationContextExpressionCapacity {
              checkArguments.append(
                Argument(
                  label: "expressionCapacity",
                  expression: IntegerLiteralExprSyntax(expressionCapacity)
                )
              )
            }
