...
public struct _AttachableImageContainer<ImageClass>: Sendable where ImageClass: AttachableAsCGImage {
  ...
  ///
  /// This property should eventually move up to ``Attachment``.
...
extension UTType {
  ...
  /// Whether or not this type is "concrete" enough to infer an image format
  /// from.
  ///
  /// The value of this property roughly represents whether the developer who
  /// specified it meant to use it as the specific image format for serializing
  /// an attachment. The caller is responsible for checking for conformance to
  /// `"public.image"`.
  var impliesSpecificImageType: Bool {
    self != .data && self != .image && isDeclared
  }
}
...
extension _AttachableImageContainer: AttachableContainer {
  ...
  public func withUnsafeBufferPointer<R>(for attachment: borrowing Attachment<Self>, _ body: (UnsafeRawBufferPointer) throws -> R) throws -> R {
    ...
    if #available(_uttypesAPI, *) {
      if let contentType, contentType.impliesSpecificImageType {
        if contentType.conforms(to: .image) {
          typeIdentifier = contentType.identifier as CFString
        } else {
          // We should have been able to use the type to infer the image format,
          // but it wasn't an image type!
          throw ImageAttachmentError.contentTypeDoesNotConformToImage
        }
      } else {
        typeIdentifier = UTType.preferred(forEncodingQuality: encodingQuality).identifier as CFString
      }
